	  ...
	  (("^install ::(.*)" all target)
	   (let 	 ((external-packages
			   '(
			     ...
			     "comet"
			     "pwiz")))
	     (if (null? (filter (lambda (pkg) (string-contains-ci target pkg)) external-packages))
		 "" all))))))
...
(define-public tpp
  (let* (
	 ...
	 (external-packages-without-source-rule
	  '("mayu" "hardklor" "pwiz"))
	 (extern-all-build
	  (string-append "EXTERN_ALL="
			 (string-join external-packages " ")))
	 (extern-all-configure
	  (string-append "EXTERN_ALL="
			 (string-join
			  (filter (lambda (s)
				    (not (member s
    ...
    (package
      ...
      (description "The Trans-Proteomic Pipeline (TPP) is open-source data analysis software for
...
validation, quantitation, visualization, and conversion of proteomics data.")
      (home-page "http://tools.proteomecenter.org/wiki/index.php?title=Software:TPP")
