...
(define openms-contrib-source
  ...
  (origin
    (method git-fetch)
    (uri (git-reference (url "https://github.com/OpenMS/contrib")
...
(define openms-minimal
  (let ((revision "1")
    ...
    (package
      ...
      (description
       "OpenMS is a C++ library for LC-MS data management and analyses. It offers an
  ...
  (package
    ...
    (arguments
     (append
      ...
      (substitute-keyword-arguments (package-arguments openms-minimal)
        ((#:configure-flags cf)
         #~`("-DPYOPENMS=ON" "-DPY_MEMLEAK_DISABLE=ON"
             ,@#$cf))
        ((#:make-flags mf)
    ...
    (native-inputs
     `(("python" ,python-wrapper)
       ...
       ;; if pip,setuptools,wheel are not here
       ;; the configure step will complain
    ...
    (inputs (modify-inputs (package-inputs openms-minimal)
                           (prepend python)))))
